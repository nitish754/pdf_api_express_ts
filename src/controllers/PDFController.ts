...
export const StudyGroupCertificatePDF: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    let mailPayload = {
      ...
      subject: `Study Group Report ${groupInfo?.group_name}`,
...
export const HostFamilyLetter: RequestHandler = async (req, res, next) => {
  ...
  const families = await HostFamily.find({ _id: { $in: host_family_id_unique } }).select('_id personal_info.full_name personal_info.family_id personal_info.address personal_info.contact personal_info.email').lean();
  ...
  try {
    ...
    const createPdf = families.forEach(async (family) => {
      ...
      let mailPayload = {
        ...
        html: `Hi ${family.personal_info.full_name}, </br> Please find the attached Report`,
        attachment: buffer
      }
      Mail.CreateEmail()?.sendEmail(mailPayload.from, mailPayload.to, mailPayload.subject, mailPayload.html, mailPayload.attachment);
